...
class Appointments extends Component {
	...
	handleFormSubmit = (event) => {
		...
		if (this.state.appointmentDoctor === "") {
			this.setState({
				appointmentDoctorError:
					"Select the doctor associated with the appointment from the drop-down list.",
			});
		}
		...
		if (
			...
			this.state.appointmentDate === "mm/dd/yyyy"
		) {
			this.setState({
				appointmentDateError:
					"Use the date picker to select the date of the appointment.",
			});
		}
		...
		if (this.state.appointmentTime === "") {
			this.setState({
				appointmentTimeError:
					"Use the time picker to select the time of the appointment in HH:MM AM/PM format.",
			});
		} else {
							...
							<Grid container spacing={24} className={classes.heading}>
								<Grid item xs={12}>
									<AppointmentsForm
										...
										handleAppointmentNameChange={
											this.handleAppointmentNameChange
										}
										handleAppointmentDoctorChange={
											this.handleAppointmentDoctorChange
										}
										...
										handleAppointmentClinicChange={
											this.handleAppointmentClinicChange
										}
