...
class Prescriptions extends Component {
	...
	loadPrescriptions = () => {
		PrescriptionsAPI.getPrescriptions()
			...
			.catch((err) =>
				console.log("loading prescriptions is not working: " + err)
			);
	};
	...
	handleFormSubmit = (event) => {
		...
		if (this.state.prescriptionDoctor === "") {
			this.setState({
				prescriptionDoctorError:
					"Select the prescribing doctor from the drop-down list.",
			});
		}

		// If the date prescribed field is empty when user submits form, show error.
		if (
			this.state.prescriptionDate === "" ||
			this.state.prescriptionDate === "mm/dd/yyyy"
		) {
			this.setState({
				prescriptionDateError:
					"Use the date picker to select the date when the prescription was prescribed.",
			});
		}
		...
		if (this.state.prescriptionDirections === "") {
			this.setState({
				prescriptionDirectionsError:
					"Enter any verbal and/or written directions received regarding this prescription. If not applicable, enter N/A.",
			});
		} else {
			...
			PrescriptionsAPI.savePrescription({
				...
				.then((res) => this.loadPrescriptions())
				.catch((err) =>
					console.log("there is an error in saving the prescription", err)
				);
							...
							<Grid container spacing={16}>
								<Grid item xs={12} sm={12} md={6}>
									<PrescriptionsForm
										...
										handlePrescriptionNameChange={
											this.handlePrescriptionNameChange
										}
										handlePrescriptionDoctorChange={
											this.handlePrescriptionDoctorChange
										}
										handlePrescriptionDateChange={
											this.handlePrescriptionDateChange
										}
										handlePrescriptionAmountChange={
											this.handlePrescriptionAmountChange
										}
										handlePrescriptionDirectionsChange={
											this.handlePrescriptionDirectionsChange
										}
										...
										prescriptionDirectionsError={
											this.state.prescriptionDirectionsError
										}
